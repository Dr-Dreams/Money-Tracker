...
const check = ()=>{
  console.log("running + checking..", editable);
  const temp = editable;
  // setEditable(null);
  return temp;
}
  ...
  useEffect(() => {
    ...
    getAllTransactions();
  }, [frequency, selectedDate, type, category, setAllTransection]);
  ...
  const handleDelete = async (record) => {
    try {
      ...
      setAllTransection((prevState) =>
        prevState.filter((transaction) => transaction._id !== record._id)
      );
    } catch (error) {
      setLoading(false);
      console.log(error);
  ...
  const handleSubmit = async (values) => {
    try {
      ...
      if (editable) {
        ...
        setAllTransection((prevState) => [...prevState, values]);
      }
      setShowModal(false);
      setEditable(null);
    } catch (error) {
  ...
  //   if (editable) {
  //     console.log("hii");
  //     form.setFieldsValue(editable); // Set form fields with the editable values
  //   }
  // }, [editable, form]);
